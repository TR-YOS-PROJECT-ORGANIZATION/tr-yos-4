...
function OneCard({ item }) {
    ...
    function moveToSelectedDepartments(id) {
        const departmentToMove = {
            en: item.en,
            facultyCode: item.facultyCode,
            id: item.id,
            tr: item.tr
        }
        let itemsList = JSON.parse(localStorage.getItem("selectedDepartmentList")) || [];
        if (itemsList.length < 4) {
            itemsList.push(departmentToMove);
        } else {
            alert("You can't compare more than 4 department")
        }
        localStorage.setItem("selectedDepartmentList", JSON.stringify(itemsList));
        setSelectedDepartments(itemsList)
    }
